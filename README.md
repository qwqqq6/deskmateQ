...
> 开机自启的无边框桌面小部件, 集成四象限待办、项目管理、工作日志(周报)、报销记录, 数据自动保存并支持备份/恢复。
...
- **四象限 TodoList**: 按艾森豪威尔矩阵 (紧急/重要) 分四象限管理待办, 支持勾选完成、就地编辑、删除; 条目可在四象限之间**拖拽移动**; 勾选完成会自动生成一条工作日志, 取消完成则回删。
- **项目管理**: 创建带颜色标签的项目, 将待办归属到具体项目; 项目详情页按象限分组展示任务, 可直接在详情页新增待办; 支持归档/恢复/删除项目; 四象限面板支持按项目筛选。
...
├── repositories/  数据访问层: models + todo/worklog/reimbursement/project 仓储
